...
import { Bell, Sun, Moon, LogOut, UserCircle, ChevronDown, CheckCircle2, Trash2, BookOpen, MessageSquare, Trophy } from 'lucide-react';
 ...
 * Gère le thème, les notifications en temps réel et le profil parent.
 */
const NavbarDash = () => {
    ...
    const getIcon = (type) => {
        switch (type) {
            case 'LECTURE_COURS': return <BookOpen className="text-orange-500" size={16} />;
            case 'EXERCICE_FINI': return <CheckCircle2 className="text-pink-500" size={16} />;
            case 'QUIZ_FINI':     return <Trophy className="text-yellow-600" size={16} />;
                        ...
                        <div className="absolute right-0 mt-2 w-80 sm:w-[26rem] bg-base-100 border border-base-300 rounded-[2rem] shadow-2xl z-[100] animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[32rem]">
                            ...
                            <div className="p-5 border-b border-base-200 flex justify-between items-center bg-base-100/50 backdrop-blur-md sticky top-0 rounded-t-[2rem]">
                                ...
                                <div className="flex items-center gap-2">
                                    {notifs.length > 0 && (
                                        <>
                                            <button onClick={markAllAsRead} className="btn btn-ghost btn-xs text-[9px] font-black uppercase tracking-wider hover:bg-base-200 rounded-lg py-1 px-2" title="Tout lire">
                                    ...
                                    notifs.map((n) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex-grow min-w-0 pr-6">
                                                <div className="flex items-center gap-2">
                                                    <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full ${n.est_lu ? 'bg-base-300' : 'text-blue-600 bg-blue-50'}`}>
                    ...
                    <div tabIndex={0} role="button" className="flex items-center gap-3 bg-base-200 hover:bg-base-300 transition-all p-1.5 pr-4 rounded-2xl border border-base-300 cursor-pointer">
                        <div className="avatar">
                            <div className="w-9 rounded-xl bg-primary text-white flex items-center justify-center font-black shadow-md uppercase">
                                {user.first_name ? user.first_name[0].toUpperCase() : user.username[0]?.toUpperCase() || 'P'}
                            </div>
                        </div>
                        <div className="hidden sm:flex flex-col items-start leading-tight">
                            <span className="text-sm font-black text-base-content">{user.first_name || user.username || 'Parent'}</span>
                            <span className="text-[10px] opacity-40 font-bold uppercase tracking-tight">Parent</span>
                        </div>
                    ...
                    <ul tabIndex={0} className="dropdown-content z-[2] menu p-2 shadow-2xl bg-base-100 rounded-2xl w-56 mt-4 border border-base-300 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <li>
                            <button 
                                ...
                                className="flex items-center gap-3 py-3 rounded-xl hover:bg-red-50 hover:text-red-500 font-bold w-full text-left"
                            >
