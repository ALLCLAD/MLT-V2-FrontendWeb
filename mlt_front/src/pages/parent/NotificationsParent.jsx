...
import { Bell, Heart, Trophy, BookOpen, CheckCircle2, MessageSquare, Loader2, Trash2, Calendar } from 'lucide-react';
...
const NotificationsParent = () => {
    ...
    // 🔗 Connexion directe au flux global de communication
    ...
    const deleteAllNotifs = async () => {
        if (!window.confirm("Voulez-vous supprimer tout l'historique de suivi de vos enfants ?")) return;
        try {
            // Aligné avec le backend : méthode POST + payload 'action'
    ...
    const getIcon = (type) => {
        switch (type) {
            case 'LECTURE_COURS': return <BookOpen className="text-orange-500" />;
            case 'EXERCICE_FINI': return <CheckCircle2 className="text-pink-500" />;
            case 'QUIZ_FINI':     return <Trophy className="text-yellow-600" />;
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 md:p-8 animate-in fade-in duration-1000">
            <div className="bg-base-100 border border-base-300 rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col min-h-[85vh]">
                <header className="p-8 md:p-12 border-b border-base-200 bg-base-100/50 backdrop-blur-md sticky top-0 z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 bg-blue-50 rounded-3xl text-blue-500"><Heart size={40} className="fill-red-500/10" /></div>
                        <div>
                            <h1 className="text-3xl font-black uppercase tracking-tighter leading-none">Suivi de mes Enfants</h1>
                            <p className="text-sm font-bold opacity-40 mt-1">Activités de vos enfants et messages en temps réel</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-10">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 opacity-20">
                            <Loader2 className="animate-spin mb-4 text-red-500" size={48} /><span className="font-black italic text-xl uppercase tracking-widest">Récupération des données...</span>
                        </div>
                    ) : notifs.length === 0 ? (
                        <div className="text-center py-32 flex flex-col items-center opacity-20">
                            <Heart size={100} strokeWidth={1} /><p className="text-2xl font-black italic mt-4 uppercase">Aucune activité récente à signaler</p>
                        </div>
                    ) : (
                                    ...
                                    <div className="grid gap-4">
                                        {items.map((n) => (
                                            <div key={n.id} onClick={() => !n.est_lu && markAsRead(n.id)}
                                                 className={`group relative p-6 md:p-8 rounded-[2.5rem] border-2 transition-all cursor-pointer flex items-center gap-8 ${n.est_lu ? 'bg-base-200/30 border-transparent opacity-50 grayscale-[0.5]' : 'bg-base-100 border-base-200 shadow-xl shadow-base-300/20 hover:border-blue-300 hover:scale-[1.01]'}`}>
                                                <div className={`p-5 rounded-[1.5rem] transition-transform group-hover:rotate-6 ${n.est_lu ? 'bg-base-200' : 'bg-base-50'}`}>{getIcon(n.type_notif)}</div>
                                                <div className="flex-grow">
                                                    <div className="flex items-center gap-2">
                                                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded-full ${n.est_lu ? 'bg-base-300' : 'text-blue-600 bg-blue-50'}`}>{n.type_notif.replace('_', ' ')}</span>
                                                        {!n.est_lu && <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-lg shadow-red-500/50"></span>}
                                                    </div>
