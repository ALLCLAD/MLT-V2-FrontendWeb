...
const NotificationsEnfant = () => {
    ...
    const getIcon = (type) => {
        switch (type) {
            case 'LECON_PUBLIEE':  return <BookOpen className="text-blue-500" />;
            case 'MESSAGE_RECU':   return <MessageSquare className="text-purple-500" />;
            ...
            <div className="bg-base-100 border border-base-300 rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col min-h-[85vh]">
                <header className="p-8 md:p-12 border-b border-base-200 bg-base-100/50 backdrop-blur-md sticky top-0 z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 bg-primary/10 rounded-3xl text-primary"><Bell size={40} /></div>
                        <div>
                            <h1 className="text-3xl font-black uppercase tracking-tighter leading-none text-base-content">Mes Notifications</h1>
                            <p className="text-sm font-bold opacity-40 mt-1 text-base-content">Tes cours, exercices et messages en temps réel</p>
                        </div>
                    </div>
                        ...
                        {notifs.length > 0 && (
                            <>
                                <button onClick={markAllAsRead} className="btn btn-ghost btn-sm text-[10px] font-black uppercase tracking-widest opacity-50 hover:opacity-100">
                                    <CheckCircle2 size={16} className="mr-2" /> Tout marquer comme lu
                                </button>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-10">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 opacity-20">
                            <Loader2 className="animate-spin mb-4" size={48} /><span className="font-black italic text-xl uppercase tracking-widest">Chargement...</span>
                        </div>
                    ) : notifs.length === 0 ? (
                        <div className="text-center py-32 flex flex-col items-center opacity-20">
                            <Bell size={100} strokeWidth={1} /><p className="text-2xl font-black italic mt-4 uppercase">Rien de neuf pour le moment</p>
                        </div>
                    ) : (
                                    ...
                                    <div className="grid gap-4">
                                        {items.map((n) => (
                                            <div key={n.id} onClick={() => !n.est_lu && markAsRead(n.id)}
                                                 className={`group relative p-6 md:p-8 rounded-[2.5rem] border-2 transition-all cursor-pointer flex items-center gap-8 ${n.est_lu ? 'bg-base-200/30 border-transparent opacity-50 grayscale-[0.5]' : 'bg-base-100 border-base-200 shadow-xl shadow-base-300/20 hover:border-red-300 hover:scale-[1.01]'}`}>
                                                <div className={`p-5 rounded-[1.5rem] transition-transform group-hover:rotate-6 ${n.est_lu ? 'bg-base-200' : 'bg-base-100'}`}>{getIcon(n.type_notif)}</div>
                                                <div className="flex-grow">
                                                    <div className="flex items-center gap-2">
                                                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded-full ${n.est_lu ? 'bg-base-300' : 'text-red-600 bg-red-50'}`}>{n.type_notif.replace('_', ' ')}</span>
                                                ...
                                                <button onClick={(e) => deleteNotif(e, n.id)} className="p-3 rounded-full opacity-0 group-hover:opacity-100 hover:bg-error/10 hover:text-error transition-all"><Trash2 size={20} /></button>
                                            </div>
                                        ))}
                                    </div>
                                </section>
